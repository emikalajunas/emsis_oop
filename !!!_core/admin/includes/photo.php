...
class Photo extends Db_object {
        ...
        protected static $db_table_fields = array('title', 'caption' ,'description', 'alternate_text' ,'filename', 'type', 'size', 'user_id');
        ...
        public $tmp_path; //temporary photo path from $_FILE
    ...
    public function set_file($file){
        ...
        if(empty($file) || !$file || !is_array($file)){
            ...
            $this->filename  = basename($file['name']);// == $_FILES['name'];
    ...
    public function save(){
        if($this->id){ //if has ID then update, else save
            $this->update();
        } else {
    ...
    public function delete_photo(){
        if($this->delete()){ //this part deletes DATA from db
            ...
            $target_path = SITE_ROOT . DS . 'admin' . DS . $this->picture_path(); //OR BELLOW
